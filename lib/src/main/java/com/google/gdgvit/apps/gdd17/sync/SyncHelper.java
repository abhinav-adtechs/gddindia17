...
import com.google.gdgvit.apps.gdd17.sync.account.Account;
import com.google.gdgvit.apps.gdd17.sync.userdata.AbstractUserDataSyncHelper;
import com.google.gdgvit.apps.gdd17.sync.userdata.UserDataSyncHelperFactory;
import com.google.gdgvit.apps.gdd17.util.LogUtils;
import com.google.gdgvit.apps.gdd17.feedback.FeedbackApiHelper;
import com.google.gdgvit.apps.gdd17.feedback.FeedbackSyncHelper;
import com.google.samples.apps.iosched.lib.BuildConfig;
import com.google.gdgvit.apps.gdd17.provider.ScheduleContract;
import com.google.gdgvit.apps.gdd17.service.DataBootstrapService;
...
import com.google.gdgvit.apps.gdd17.util.ConnectivityUtils;
...
public class SyncHelper {
    ...
    public static void requestManualSync(boolean userDataSyncOnly) {
        LogUtils.LOGD(TAG, "Requesting manual sync for account. userDataSyncOnly=" + userDataSyncOnly);
        ...
        if (active) {
            LogUtils.LOGD(TAG, "Cancelling previously active sync.");
    ...
    private boolean doUserDataSync(SyncResult syncResult) throws IOException {
        ...
        if (TextUtils.isEmpty(accountName)) {
            LogUtils.LOGD(TAG, "Not attempting userdata sync because user is not signed in");
            return false;
        }
   ...
   private static long calculateRecommendedSyncInterval(final Context context) {
    ...
    public static void updateSyncInterval(final Context context) {
        android.accounts.Account account = Account.getAccount();
        LogUtils.LOGD(TAG, "Checking sync interval");
    ...
    public static class MinimalRequestLogger implements RequestLogger {
        ...
        public void logRequest(final HttpURLConnection urlConnection, final Object o)
            ...
            try {
                ...
                LogUtils.LOGW(TAG, "HTTPRequest to " + url.getHost());
            } catch (Throwable e) {
                LogUtils.LOGI(TAG, "Exception while logging http request.");
            }
        ...
        public void logResponse(final HttpResponse httpResponse) {
            try {
                ...
                LogUtils.LOGW(TAG, "HTTPResponse from " + url.getHost() + " had return status " + httpResponse.getStatus());
            } catch (Throwable e) {
                LogUtils.LOGI(TAG, "Exception while logging http response.");
            }
        }
    }
}
