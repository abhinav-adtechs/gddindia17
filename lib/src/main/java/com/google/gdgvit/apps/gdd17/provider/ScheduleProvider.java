...
package com.google.gdgvit.apps.gdd17.provider;
...
import com.google.gdgvit.apps.gdd17.Config;
import com.google.gdgvit.apps.gdd17.appwidget.ScheduleWidgetProvider;
import com.google.gdgvit.apps.gdd17.sync.ConferenceDataHandler;
import com.google.gdgvit.apps.gdd17.sync.SyncHelper;
import com.google.gdgvit.apps.gdd17.sync.userdata.AbstractUserDataSyncHelper;
import com.google.gdgvit.apps.gdd17.util.AccountUtils;
import com.google.gdgvit.apps.gdd17.util.LogUtils;
import com.google.gdgvit.apps.gdd17.settings.SettingsUtils;
import com.google.gdgvit.apps.gdd17.util.SelectionBuilder;
...
public class ScheduleProvider extends ContentProvider {
    ...
    private void addTagsFilter(SelectionBuilder builder, String tagsFilter, String numCategories) {
        ...
        if (requiredTags.length == 0) {
            ...
            builder.where(ScheduleContract.Tags.TAG_ID + "=?", requiredTags[0]);
        } else {
            ...
            builder.where(ScheduleContract.Tags.TAG_ID + " IN " + questionMarkTuple, requiredTags);
            builder.having(
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
        ...
        String categories = uri.getQueryParameter(ScheduleContract.Sessions.QUERY_PARAMETER_CATEGORIES);
        ...
        switch (matchingUriEnum) {
            ...
            case SEARCH_TOPICS_SESSIONS: {
                ...
                Cursor tags = query(ScheduleContract.Tags.CONTENT_URI, ScheduleContract.SearchTopicsSessions.TOPIC_TAG_PROJECTION,
                        ScheduleContract.SearchTopicsSessions.TOPIC_TAG_SELECTION,
                        new String[] {Config.Tags.CATEGORY_TRACK, selectionArg + "%"},
                        ScheduleContract.SearchTopicsSessions.TOPIC_TAG_SORT);
                ...
                if (selectionArgs[0] != null) { // dont query if there was no selectionArg.
                    search = query(ScheduleContract.Sessions.buildSearchUri(selectionArg),
                            ScheduleContract.SearchTopicsSessions.SEARCH_SESSIONS_PROJECTION,
                            null, null,
     ...
     *
     * @param tags Cursor with the projection {@link ScheduleContract.SearchTopicsSessions#TOPIC_TAG_PROJECTION}.
     * @param search Cursor with the projection
     *              {@link ScheduleContract.SearchTopicsSessions#SEARCH_SESSIONS_PROJECTION}.
     * @return Returns a MatrixCursor always with {@link ScheduleContract.SearchTopicsSessions#DEFAULT_PROJECTION}
     */
    private Cursor createMergedSearchCursor(Cursor tags, Cursor search) {
        ...
        MatrixCursor matrixCursor = new MatrixCursor(
                ScheduleContract.SearchTopicsSessions.DEFAULT_PROJECTION, maxCount);
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        switch (matchingUriEnum) {
            ...
            case TAGS: {
                return ScheduleContract.Tags.buildTagUri(values.getAsString(ScheduleContract.Tags.TAG_ID));
            }
            case ROOMS: {
            ...
            case SESSIONS_ID_RELATED: {
                values.put(ScheduleContract.Sessions.SESSION_ID, ScheduleContract.Sessions.getSessionId(uri));
                db.insertOrThrow(ScheduleDatabase.Tables.RELATED_SESSIONS, null, values);
                notifyChange(uri);
                return uri;
            }
            case MY_SCHEDULE: {
                values.put(ScheduleContract.MySchedule.MY_SCHEDULE_ACCOUNT_NAME, getCurrentAccountName(uri, false));
                db.insertOrThrow(ScheduleDatabase.Tables.MY_SCHEDULE, null, values);
                notifyChange(uri);
                Uri sessionUri = ScheduleContract.Sessions.buildSessionUri(
                        values.getAsString(ScheduleContract.MyScheduleColumns.SESSION_ID));
                notifyChange(sessionUri);
                // Queries for sessions in user's schedule are affected by this change.
                notifyChange(ScheduleContract.Sessions.CONTENT_MY_SCHEDULE_URI);
                return sessionUri;
            }
            case MY_RESERVATIONS: {
                values.put(ScheduleContract.MyReservations.MY_RESERVATION_ACCOUNT_NAME, getCurrentAccountName(uri, false));
                db.insertOrThrow(ScheduleDatabase.Tables.MY_RESERVATIONS, null, values);
                notifyChange(uri);
                Uri sessionUri = ScheduleContract.Sessions.buildSessionUri(
                        values.getAsString(ScheduleContract.MyReservationColumns.SESSION_ID));
                notifyChange(sessionUri);
                // Queries for sessions in user's schedule are affected by this change.
                notifyChange(ScheduleContract.Sessions.CONTENT_MY_SCHEDULE_URI);
                return sessionUri;
            ...
            case MY_VIEWED_VIDEOS: {
                values.put(ScheduleContract.MyViewedVideos.MY_VIEWED_VIDEOS_ACCOUNT_NAME,
                        getCurrentAccountName(uri, false));
                db.insertOrThrow(ScheduleDatabase.Tables.MY_VIEWED_VIDEO, null, values);
                notifyChange(uri);
                Uri videoUri = ScheduleContract.Videos.buildVideoUri(
                        values.getAsString(ScheduleContract.MyViewedVideos.VIDEO_ID));
                notifyChange(videoUri);
                return videoUri;
            }
            case MY_FEEDBACK_SUBMITTED: {
                values.put(ScheduleContract.MyFeedbackSubmitted.MY_FEEDBACK_SUBMITTED_ACCOUNT_NAME,
                        getCurrentAccountName(uri, false));
                db.insertOrThrow(ScheduleDatabase.Tables.MY_FEEDBACK_SUBMITTED, null, values);
                notifyChange(uri);
                Uri sessionUri = ScheduleContract.Sessions.buildSessionUri(
                        values.getAsString(ScheduleContract.MyFeedbackSubmitted.SESSION_ID));
                notifyChange(sessionUri);
                return sessionUri;
            }
            case SPEAKERS: {
            ...
            case ANNOUNCEMENTS: {
                return ScheduleContract.Announcements.buildAnnouncementUri(values
                        .getAsString(ScheduleContract.Announcements.ANNOUNCEMENT_ID));
            }
            case SEARCH_SUGGEST: {
            ...
            case FEEDBACK_FOR_SESSION: {
                return ScheduleContract.Feedback.buildFeedbackUri(values.getAsString(ScheduleContract.Feedback.SESSION_ID));
            }
            case HASHTAGS: {
                return ScheduleContract.Hashtags.buildHashtagUri(values.getAsString(ScheduleContract.Hashtags.HASHTAG_NAME));
            }
            case VIDEOS: {
                return ScheduleContract.Videos.buildVideoUri(values.getAsString(ScheduleContract.Videos.VIDEO_ID));
            }
            default: {
    ...
    public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        if (matchingUriEnum == ScheduleUriEnum.MY_SCHEDULE) {
            if (!isAccountUpdateAllowed) {
                values.remove(ScheduleContract.MySchedule.MY_SCHEDULE_ACCOUNT_NAME);
            }
            builder.where(ScheduleContract.MySchedule.MY_SCHEDULE_ACCOUNT_NAME + "=?", accountName);
            // Also notify session listeners.
            notifyUri = ScheduleContract.Sessions.CONTENT_MY_SCHEDULE_URI;
        }
        if (matchingUriEnum == ScheduleUriEnum.MY_RESERVATIONS) {
            values.remove(ScheduleContract.MyReservations.MY_RESERVATION_ACCOUNT_NAME);
            builder.where(ScheduleContract.MyReservations.MY_RESERVATION_ACCOUNT_NAME + "=?", accountName);
        }
        if (matchingUriEnum == ScheduleUriEnum.MY_VIEWED_VIDEOS) {
            values.remove(ScheduleContract.MyViewedVideos.MY_VIEWED_VIDEOS_ACCOUNT_NAME);
            builder.where(ScheduleContract.MyViewedVideos.MY_VIEWED_VIDEOS_ACCOUNT_NAME + "=?", accountName);
        }
        if (matchingUriEnum == ScheduleUriEnum.MY_FEEDBACK_SUBMITTED) {
            if (!isAccountUpdateAllowed) {
                values.remove(ScheduleContract.MyFeedbackSubmitted.MY_FEEDBACK_SUBMITTED_ACCOUNT_NAME);
            }
            builder.where(ScheduleContract.MyFeedbackSubmitted.MY_FEEDBACK_SUBMITTED_ACCOUNT_NAME + "=?",
                    accountName);
        }
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        if (matchingUriEnum == ScheduleUriEnum.MY_FEEDBACK_SUBMITTED) {
            builder.where(
                    ScheduleContract.MyFeedbackSubmitted.MY_FEEDBACK_SUBMITTED_ACCOUNT_NAME + "=?", accountName);
        }
    ...
    private SelectionBuilder buildSimpleSelection(Uri uri) {
        ...
        switch (matchingUriEnum) {
            ...
            case BLOCKS_ID: {
                final String blockId = ScheduleContract.Blocks.getBlockId(uri);
                return builder.table(ScheduleDatabase.Tables.BLOCKS)
                        .where(ScheduleContract.Blocks.BLOCK_ID + "=?", blockId);
            }
            case TAGS_ID: {
                final String tagId = ScheduleContract.Tags.getTagId(uri);
                return builder.table(ScheduleDatabase.Tables.TAGS)
                        .where(ScheduleContract.Tags.TAG_ID + "=?", tagId);
            }
            case ROOMS_ID: {
                final String roomId = ScheduleContract.Rooms.getRoomId(uri);
                return builder.table(ScheduleDatabase.Tables.ROOMS)
                        .where(ScheduleContract.Rooms.ROOM_ID + "=?", roomId);
            }
            case SESSIONS_ID: {
                final String sessionId = ScheduleContract.Sessions.getSessionId(uri);
            ...
            case MY_SCHEDULE: {
                return builder.table(ScheduleDatabase.Tables.MY_SCHEDULE)
                        .where(ScheduleContract.MySchedule.MY_SCHEDULE_ACCOUNT_NAME + "=?",
                                getCurrentAccountName(uri, false));
            }
            case MY_RESERVATIONS: {
                return builder.table(ScheduleDatabase.Tables.MY_RESERVATIONS)
                        .where(ScheduleContract.MyReservations.MY_RESERVATION_ACCOUNT_NAME + "=?",
                                getCurrentAccountName(uri, false));
            }
            case MY_VIEWED_VIDEOS: {
                return builder.table(ScheduleDatabase.Tables.MY_VIEWED_VIDEO)
                        .where(ScheduleContract.MyViewedVideos.MY_VIEWED_VIDEOS_ACCOUNT_NAME + "=?",
                                getCurrentAccountName(uri, false));
            }
            case MY_FEEDBACK_SUBMITTED: {
                return builder.table(ScheduleDatabase.Tables.MY_FEEDBACK_SUBMITTED)
                        .where(ScheduleContract.MyFeedbackSubmitted.MY_FEEDBACK_SUBMITTED_ACCOUNT_NAME + "=?",
                                getCurrentAccountName(uri, false));
            }
            case SPEAKERS_ID: {
                final String speakerId = ScheduleContract.Speakers.getSpeakerId(uri);
            ...
            case ANNOUNCEMENTS_ID: {
                final String announcementId = ScheduleContract.Announcements.getAnnouncementId(uri);
                return builder.table(ScheduleDatabase.Tables.ANNOUNCEMENTS)
                        .where(ScheduleContract.Announcements.ANNOUNCEMENT_ID + "=?", announcementId);
            }
            case FEEDBACK_FOR_SESSION: {
                final String session_id = ScheduleContract.Feedback.getSessionId(uri);
                return builder.table(ScheduleDatabase.Tables.FEEDBACK)
                        .where(ScheduleContract.Feedback.SESSION_ID + "=?", session_id);
            }
            case FEEDBACK_ALL: {
                return builder.table(ScheduleDatabase.Tables.FEEDBACK);
            }
            case HASHTAGS_NAME: {
                final String hashtagName = ScheduleContract.Hashtags.getHashtagName(uri);
                return builder.table(ScheduleDatabase.Tables.HASHTAGS)
                        .where(ScheduleContract.Hashtags.HASHTAG_NAME + "=?", hashtagName);
            }
            case VIDEOS_ID: {
                final String videoId = ScheduleContract.Videos.getVideoId(uri);
                return builder.table(ScheduleDatabase.Tables.VIDEOS).where(ScheduleContract.Videos.VIDEO_ID + "=?", videoId);
            }
            default: {
        ...
        switch (matchingUriEnum) {
            ...
            case SESSIONS: {
                ...
                return builder
                        ...
                        .map(ScheduleContract.Sessions.SESSION_IN_MY_SCHEDULE, "IFNULL(in_schedule, 0)")
                        .map(ScheduleContract.Sessions.SESSION_RESERVATION_STATUS, "IFNULL(" + ScheduleContract.MyReservations.
            ...
            case SESSIONS_COUNTER: {
                return builder
                        ...
                        .map(ScheduleContract.Sessions.SESSION_INTERVAL_COUNT, "count(1)")
                        .map(ScheduleContract.Sessions.SESSION_IN_MY_SCHEDULE, "IFNULL(in_schedule, 0)")
                        .map(ScheduleContract.Sessions.SESSION_RESERVATION_STATUS, "IFNULL(" + ScheduleContract.MyReservations.
                                MY_RESERVATION_STATUS + ", -1)")
                        .groupBy(ScheduleContract.Sessions.SESSION_START + ", " + ScheduleContract.Sessions.SESSION_END);
            }
            case SESSIONS_MY_SCHEDULE: {
                return builder.table(ScheduleDatabase.Tables.SESSIONS_JOIN_ROOMS_TAGS_FEEDBACK_MYSCHEDULE,
                        ...
                        .mapToTable(ScheduleContract.Sessions.ROOM_ID, ScheduleDatabase.Tables.SESSIONS)
                        .mapToTable(ScheduleContract.Sessions.SESSION_ID, ScheduleDatabase.Tables.SESSIONS)
                        .map(ScheduleContract.Sessions.HAS_GIVEN_FEEDBACK, Subquery.SESSION_HAS_GIVEN_FEEDBACK)
                        .map(ScheduleContract.Sessions.SESSION_IN_MY_SCHEDULE, "IFNULL(in_schedule, 0)")
                        .map(ScheduleContract.Sessions.SESSION_RESERVATION_STATUS, "IFNULL(" + ScheduleContract.MyReservations.
                                MY_RESERVATION_STATUS + ", -1)")
                        .where(ScheduleContract.Sessions.IN_SCHEDULE_SELECTION)
                        .groupBy(Qualified.SESSIONS_SESSION_ID);
            }
            case SESSIONS_UNSCHEDULED: {
                final long[] interval = ScheduleContract.Sessions.getInterval(uri);
                ...
                return builder.table(ScheduleDatabase.Tables.SESSIONS_JOIN_ROOMS_TAGS_FEEDBACK_MYSCHEDULE,
                        ...
                        .map(ScheduleContract.Sessions.SESSION_RESERVATION_STATUS, "IFNULL(" + ScheduleContract.MyReservations.
                                MY_RESERVATION_STATUS + ", -1)")
                        .where(ScheduleContract.Sessions.NOT_IN_SCHEDULE_SELECTION)
                        .where(ScheduleContract.Sessions.SESSION_START + ">=?", String.valueOf(interval[0]))
                        .where(ScheduleContract.Sessions.SESSION_START + "<?", String.valueOf(interval[1]))
                        .groupBy(Qualified.SESSIONS_SESSION_ID);
            }
            case SESSIONS_SEARCH: {
                final String query = ScheduleContract.Sessions.getSearchQuery(uri);
            ...
            case SESSIONS_AT: {
                ...
                return builder.table(ScheduleDatabase.Tables.SESSIONS_JOIN_ROOMS, accountName, accountName)
                        .mapToTable(ScheduleContract.Sessions._ID, ScheduleDatabase.Tables.SESSIONS)
                        .mapToTable(ScheduleContract.Sessions.ROOM_ID, ScheduleDatabase.Tables.SESSIONS)
                        .where(ScheduleContract.Sessions.SESSION_START + "<=?", time)
                        .where(ScheduleContract.Sessions.SESSION_END + ">=?", time);
            }
            case SESSIONS_ID: {
                final String sessionId = ScheduleContract.Sessions.getSessionId(uri);
                return builder.table(ScheduleDatabase.Tables.SESSIONS_JOIN_ROOMS, accountName, accountName)
            ...
            case SESSIONS_ID_TAGS: {
                ...
                return builder.table(ScheduleDatabase.Tables.SESSIONS_TAGS_JOIN_TAGS)
                        .mapToTable(ScheduleContract.Tags._ID, ScheduleDatabase.Tables.TAGS)
                        .mapToTable(ScheduleContract.Tags.TAG_ID, ScheduleDatabase.Tables.TAGS)
            ...
            case SESSIONS_ID_RELATED: {
                ...
                return builder
                        ...
                        .mapToTable(ScheduleContract.Sessions.SESSION_ID, ScheduleDatabase.Tables.SESSIONS)
                        .map(ScheduleContract.Sessions.SESSION_IN_MY_SCHEDULE, "IFNULL(in_schedule, 0)")
                        .map(ScheduleContract.Sessions.SESSION_RESERVATION_STATUS, "IFNULL(" + ScheduleContract
            ...
            case SESSIONS_ROOM_AFTER: {
                final String room = ScheduleContract.Sessions.getRoom(uri);
                final String time = ScheduleContract.Sessions.getAfterForRoom(uri);
                return builder.table(ScheduleDatabase.Tables.SESSIONS_JOIN_ROOMS_TAGS, accountName, accountName)
                        ...
                        .where("(" + ScheduleContract.Sessions.SESSION_START + "<= ? AND " + ScheduleContract.Sessions.SESSION_END +
                                        " >= ?) OR (" + ScheduleContract.Sessions.SESSION_START + " >= ?)", time,
                                time,
                                time)
                        .map(ScheduleContract.Sessions.SESSION_IN_MY_SCHEDULE, "IFNULL(in_schedule, 0)")
                        .map(ScheduleContract.Sessions.SESSION_RESERVATION_STATUS, "IFNULL(" + ScheduleContract
            ...
            case SESSIONS_AFTER: {
                final String time = ScheduleContract.Sessions.getAfter(uri);
                return builder.table(ScheduleDatabase.Tables.SESSIONS_JOIN_ROOMS_TAGS, accountName, accountName)
                        .mapToTable(ScheduleContract.Sessions._ID, ScheduleDatabase.Tables.SESSIONS)
                        .mapToTable(ScheduleContract.Sessions.SESSION_ID, ScheduleDatabase.Tables.SESSIONS)
                        .mapToTable(ScheduleContract.Sessions.ROOM_ID, ScheduleDatabase.Tables.SESSIONS)
            ...
            case FEEDBACK_FOR_SESSION: {
                final String sessionId = ScheduleContract.Feedback.getSessionId(uri);
                return builder.table(ScheduleDatabase.Tables.FEEDBACK)
                        .where(ScheduleContract.Feedback.SESSION_ID + "=?", sessionId);
            }
            case FEEDBACK_ALL: {
                return builder.table(ScheduleDatabase.Tables.FEEDBACK);
            }
            ...
            case HASHTAGS_NAME: {
                final String hashtagName = ScheduleContract.Hashtags.getHashtagName(uri);
                return builder.table(ScheduleDatabase.Tables.HASHTAGS)
                        .where(ScheduleContract.HashtagColumns.HASHTAG_NAME + "=?", hashtagName);
            }
            case VIDEOS: {
            ...
            case VIDEOS_ID: {
                final String videoId = ScheduleContract.Videos.getVideoId(uri);
                return builder.table(ScheduleDatabase.Tables.VIDEOS)
                        .where(ScheduleContract.VideoColumns.VIDEO_ID + "=?", videoId);
            }
            default: {
                throw new UnsupportedOperationException("Unknown uri: " + uri);
            }
    ...
    private interface Subquery {
        ...
        String RELATED_SESSIONS_SELECTION = Qualified.SESSIONS_SESSION_ID + " IN (SELECT "
                + ScheduleContract.Sessions.RELATED_SESSION_ID + " FROM " + ScheduleDatabase.Tables.RELATED_SESSIONS + " WHERE "
    ...
    private interface Qualified {
        String SESSIONS_SESSION_ID = ScheduleDatabase.Tables.SESSIONS + "." + ScheduleContract.Sessions.SESSION_ID;
        String SESSIONS_ROOM_ID = ScheduleDatabase.Tables.SESSIONS + "." + ScheduleContract.Sessions.ROOM_ID;
        ...
        String FEEDBACK_SESSION_ID = ScheduleDatabase.Tables.FEEDBACK + "." + ScheduleContract.Feedback.SESSION_ID;
    }
}
